...
public:
    ...
    Sinhvien(string _maSV, string _hoTen, float _diemTB, Date _sinhNhat)
    {
        maSV = _maSV;
        hoTen = _hoTen;
        diemTB = _diemTB;
...
void Sinhvien ::inThongTin()
{
    ...
    cout << sinhNhat.ngay << "\\" << sinhNhat.thang << "\\" << sinhNhat.nam;
    cout << endl;
}
...
public:
    ...
    vector<Sinhvien> timSVTheoNgayThangNamSinh(Date sn);

    vector<Sinhvien> timSVSinhThangM(int thang);
};
...
{
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            cin >> nam;
            cin.ignore();
            Date sinhNhat(ngay, thang, nam);
            Sinhvien sv(maSV, hoTen, diem, sinhNhat);
...
void QuanLySinhVien ::khoiTaoNgauNhien()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        int ngay = (int)(rand() % 28 + 1);
        int thang = (int)(rand() % 12 + 1);
        int nam = (int)(rand() % 5 + 1999);
        Date sinhNhat(ngay, thang, nam);
        Sinhvien sv(maSV, hoTen, diemTB, sinhNhat);
        danhSachSinhVien.push_back(sv);
    }
}
...
vector<Sinhvien> QuanLySinhVien ::timSVTheoNgayThangNamSinh(Date sn)
{
    vector<Sinhvien> danhSachSV;
    for (auto sv : danhSachSinhVien)
    {
        if (sv.sinhNhat.ngay == sn.ngay && sv.sinhNhat.thang == sn.thang && sv.sinhNhat.nam == sn.nam)
        {
...
vector<Sinhvien> QuanLySinhVien ::timSVSinhThangM(int thang)
{
    vector<Sinhvien> danhSachSV;
    for (auto sv : danhSachSinhVien)
    {
        if (sv.sinhNhat.thang == thang)
        {
            danhSachSV.push_back(sv);
        }
    }
...
int main()
{
    ...
    do
    {
        ...
        cout << "11. Tim cac sinh vien theo ngay thang nam sinh" << endl;
        cout << "12. Tim cac sinh vien sinh vao thang m" << endl;
        ...
        case 11:
            ...
            cout <<"Nhap nam: ";
            cin >> date.nam;
            xuatThongTin(Quanly.timSVTheoNgayThangNamSinh(date));
            break;
        case 12:
            cout <<"Nhap thang: ";
            cin >> thang;
            xuatThongTin(Quanly.timSVSinhThangM(thang));
            break;
        case 0:
